"""
Reliability Risk Scoring Engine — RAW MILL VERSION (VRM-primary)
-----------------------------------------------------------------------
Same dual-model architecture as cement mill / coal mill versions.
Severity table reflects VRM-primary framing -- see
reliability_framework_raw_mills.md Section 0/3 for why the grinding
table/rollers (top severity here) is deliberately left unmodeled rather
than forced through the bearing-fault proxy.

- CWRU-trained Random Forest -> bearing components (main table, fan, separator)
- Gearbox-trained Random Forest -> girth gear / pinion / gearbox
- Grinding table/rollers, hydraulic loading system, nozzle/dam ring -> unmodeled

Risk Score = P_fault x W_severity x W_detection x 100

Outputs:
  - /home/claude/risk_scores_raw_mills.csv
  - /home/claude/unmodeled_components_raw_mills.csv
  - /home/claude/risk_matrix_raw_mills.png
  - /home/claude/risk_tier_distribution_raw_mills.png
"""
...
BEARING_COMPONENTS = {
    "Main Table Bearing":                     0.85,
    "Mill Fan (Hot Gas Draft Fan) Bearing":    0.55,
    "Separator / Classifier Drive Bearing":    0.50,
}
GEARBOX_COMPONENT = {"Girth Gear / Pinion / Gearbox": 0.90}

UNMODELED_COMPONENTS = {
    "Grinding Table / Rollers (VRM)":     1.00,
    "Hydraulic Roller-Loading System":    0.70,
    "Nozzle Ring / Dam Ring Wear":        0.40,
}
...
def main():
    ...
    combined.to_csv("/home/claude/risk_scores_raw_mills.csv", index=False)
    ...
    unmodeled.to_csv("/home/claude/unmodeled_components_raw_mills.csv", index=False)
    ...
    print("\nUnmodeled components (severity-only, no P_fault) -- NOTE: top-severity")
    print("component (Grinding Table/Rollers, W_severity=1.00) is in this list:")
    ...
    ax.legend(handles=tier_handles + source_handles, loc="lower right", fontsize=8)
    ax.set_xlabel("P(fault) — model predicted probability")
    ax.set_ylabel("Severity weight (raw mill component criticality)")
    ax.set_title("Raw Mill Risk Matrix: Circles=Bearing Model, Triangles=Gearbox Model\n(Note: severity 1.00 component is unmodeled -- not shown here)")
    plt.tight_layout()
    plt.savefig("/home/claude/risk_matrix_raw_mills.png", dpi=150)
    plt.close()
    ...
    ax.set_title("Distribution of Raw Mill Readings by Risk Tier\n(Modeled Components Only)")
    ax.set_ylabel("Number of readings")
    plt.tight_layout()
    plt.savefig("/home/claude/risk_tier_distribution_raw_mills.png", dpi=150)
    plt.close()

    print("\nSaved: risk_scores_raw_mills.csv, unmodeled_components_raw_mills.csv, "
          "risk_matrix_raw_mills.png, risk_tier_distribution_raw_mills.png")
